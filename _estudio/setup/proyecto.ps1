function Assert-ProjectRoot {
    ...
    $required = @(
        "AGENTS.md",
        "_estudio\soporte\scripts\compilar_y_grabar.bat",
...
function Ensure-ProjectFolders {
    ...
    foreach ($folder in @("_estudio\logs", "Ejercicios", "usuario")) {
...
function Ensure-AgentRuntimeTools {
    ...
    $runtimePath = Join-Path $RepoRoot "_estudio\soporte\runtime"
    $launcherSource = Join-Path $RepoRoot "_estudio\soporte\consola\output_launcher.c"
    $launcherBinary = Join-Path $runtimePath "_output.exe"
    $conioSource = Join-Path $RepoRoot "_estudio\soporte\consola\conio.c"
    $conioObject = Join-Path $runtimePath "conio_support.o"
    $includeDir = Join-Path $RepoRoot "_estudio\include"
    ...
    if ($SoloVerificar) {
        Write-SetupInfo "[SoloVerificar] Compilaria _estudio\soporte\runtime\_output.exe y _estudio\soporte\runtime\conio_support.o con GCC."
        return
    }
    ...
    if ($needsLauncherBuild) {
        Invoke-SetupCommand `
            ...
            -Description "Compilando _estudio\soporte\runtime\_output.exe..." `
            -SoloVerificar:$false

        Write-SetupSuccess "_estudio\soporte\runtime\_output.exe listo."
    } else {
        Write-SetupSuccess "_estudio\soporte\runtime\_output.exe ya esta actualizado."
    }
    ...
    if ($needsConioBuild) {
        Invoke-SetupCommand `
            ...
            -Description "Compilando _estudio\soporte\runtime\conio_support.o..." `
            -SoloVerificar:$false

        Write-SetupSuccess "_estudio\soporte\runtime\conio_support.o listo."
    } else {
        Write-SetupSuccess "_estudio\soporte\runtime\conio_support.o ya esta actualizado."
    }
}
...
function Get-LegacyProjectUserRegistryPath {
    param([string]$RepoRoot)
    return (Join-Path $RepoRoot "_estudio\usuarios\registro.json")
}
...
function Resolve-ProjectUserDirectory {
    ...
    if (Test-UsableGitIdentityValue -Value $UsuarioSlug) {
        $legacy = Join-Path $RepoRoot ("_estudio\usuarios\" + (ConvertTo-ProjectUserSlug -Value $UsuarioSlug))
...
function Move-ProjectUserDirectory {
    ...
    $oldDir = Join-Path $RepoRoot ("_estudio\usuarios\" + $PreviousUsuarioSlug)
    $newDir = Join-Path $RepoRoot "usuario"
    if (-not (Test-Path -LiteralPath $oldDir)) {
        return
    }
    if (Test-Path -LiteralPath $newDir) {
        Write-SetupWarning "Ya existe usuario; no se movera _estudio\usuarios\$PreviousUsuarioSlug automaticamente."
        return
    }

    if ($SoloVerificar) {
        Write-SetupInfo "[SoloVerificar] Migraria _estudio\usuarios\$PreviousUsuarioSlug a usuario."
        return
    }

    Move-Item -LiteralPath $oldDir -Destination $newDir
    Write-SetupSuccess "Carpeta de usuario migrada: _estudio\usuarios\$PreviousUsuarioSlug -> usuario."
}
