function Ensure-GccToolchain {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        ...
        $gccLog = Join-Path $RepoRoot ("_estudio\logs\setup\gcc_intento_{0}_{1}.log" -f $attempt, $timestamp)
        $gccScript = Join-Path $RepoRoot ("_estudio\logs\setup\gcc_instalar_{0}_{1}.sh" -f $attempt, $timestamp)
