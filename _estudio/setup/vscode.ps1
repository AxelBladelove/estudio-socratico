...
function Install-EstudioExercismExtension {
    ...
    $extensionDir = Join-Path $RepoRoot "_estudio\soporte\vscode\estudio-exercism"
    $packagePath = Join-Path $extensionDir "package.json"
    if (-not (Test-Path $packagePath)) {
        Write-SetupWarning "No existe la extension local en _estudio\soporte\vscode\estudio-exercism."
        return
    }
    ...
    $vsixDir = Join-Path $RepoRoot "_estudio\soporte\runtime\vscode"
