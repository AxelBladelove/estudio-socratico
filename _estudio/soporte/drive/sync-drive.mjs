...
const configPath = path.join(repoRoot, "_estudio", "soporte", "drive", "drive.config.json");
const catalogRoot = path.join(repoRoot, "_estudio", "soporte", "exercism", "catalogs");
...
function findRepoRoot(start) {
  ...
  while (true) {
    if (existsSync(path.join(current, "package.json")) && existsSync(path.join(current, "_estudio", "soporte"))) {
      return current;
    }
