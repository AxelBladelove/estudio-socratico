...
set "RUNTIME_DIR=%REPO_ROOT%\_estudio\soporte\runtime"
...
set "INCLUDE_DIR=%REPO_ROOT%\_estudio\include"
...
if not exist "%BUILD_CONTEXT_SCRIPT%" (
    echo [ERROR] No existe _estudio\soporte\scripts\resolve_build_context.ps1. Verifica la integridad del repo.
    exit /b 1
)

if not exist "%FINALIZE_SCRIPT%" (
    echo [ERROR] No existe _estudio\soporte\scripts\finalizar_intento.bat. Verifica la integridad del repo.
    exit /b 1
)
...
set "USUARIO_LEGACY_DIR=%REPO_ROOT%\_estudio\usuarios\%USUARIO_SLUG%"
...
if not exist "%CONIO_SRC%" (
    echo [ERROR] No existe _estudio\soporte\consola\conio.c. Verifica la integridad del repo.
    exit /b 1
)

if not exist "%OUTPUT_LAUNCHER_SRC%" (
    echo [ERROR] No existe _estudio\soporte\consola\output_launcher.c. Verifica la integridad del repo.
    exit /b 1
)
...
if "%REBUILD_CONIO_OBJ%"=="1" (
    echo [INFO] Compilando cache local _estudio\soporte\runtime\conio_support.o...
    "%GCC_EXE%" "%CONIO_SRC%" -I "%INCLUDE_DIR%" -c -o "%CONIO_OBJ%" -std=c99 -Wall -Wextra >nul 2>&1
    if exist "%CONIO_OBJ%" (
        echo [OK] Cache de conio lista.
    ) else (
        echo [ERROR] No se pudo compilar _estudio\soporte\runtime\conio_support.o.
        exit /b 1
    )
)
