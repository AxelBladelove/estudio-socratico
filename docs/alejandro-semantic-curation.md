...
- Una **taxonomía estricta** (`soporte/alejandro/semantic-taxonomy.json`). No puede usar filtros que no existan ahí.
...
La taxonomía está definida en `soporte/alejandro/semantic-taxonomy.json`. El pipeline verifica estrictamente que los filtros devueltos por la IA pertenezcan a esa lista. Si no lo son, se rechaza y se marca como error.

## 5. Elección de Iconos

No le pedimos a la IA que invente código SVG (lo cual suele fallar o generar gráficos inconsistentes). La IA solo genera el `iconConcept` (ej: "calculator"). Luego, un script dedicado (`soporte/alejandro/iconify-icons.mjs`) consulta la API pública de **Iconify** y descarga un icono oficial de colecciones limpias como Lucide o Tabler, garantizando un aspecto moderno y profesional.
